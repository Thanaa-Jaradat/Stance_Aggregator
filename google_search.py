...
def search_claim(claim):
    ...
    urls = []
    for result in search(claim, tld="com", num=10, stop=10, pause=randrange(5,10), lang='en', start=0):   #   ==>>> using random number generator to generate the value of pause to avoid being blocked by google
        urls.append(result)
    articles =[]
    for url in urls:
        article = Article(url)
